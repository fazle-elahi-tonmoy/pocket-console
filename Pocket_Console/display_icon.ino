void display_icon(byte base, byte peak, byte a) {
  ...
  if (base == 1) {
    ...
    oled.drawBitmap(100, 4, mouse_icon, 24, 24, 1);
  }

  if (base == 2 || peak == 2) {
    (base == 2) ? y_pos = 4 : y_pos = 36;
    if (a >= 5 && a <= 8) oled.drawRoundRect((a - 5) * 32, y_pos - 4, 32, 32, 8, 1);
    oled.drawBitmap(4, y_pos, stopwatch_icon, 24, 24, 1);
    oled.drawBitmap(36, y_pos, timer_icon, 24, 24, 1);
    oled.drawBitmap(68, y_pos, flappy_bird_icon, 24, 24, 1);
    oled.drawBitmap(100, y_pos, space_ship_icon, 24, 24, 1);
  }

  if (base == 3 || peak == 3) {
    (base == 3) ? y_pos = 4 : y_pos = 36;
    if (a >= 9 && a <= 12) oled.drawRoundRect((a - 9) * 32, y_pos - 4, 32, 32, 8, 1);
    oled.drawBitmap(4, y_pos,snake_icon , 24, 24, 1);
    oled.drawBitmap(36, y_pos, dxball_icon, 24, 24, 1);
    oled.drawBitmap(68, y_pos, tetris_icon, 24, 24, 1);
  }
}